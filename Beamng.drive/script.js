...
(function() {
    ...
    function loadAdBlueMediaScript() {
        ...
        if (!window.VpsTI_gyC_dDLjRc) {
            const configScript = document.createElement('script');
            configScript.type = 'text/javascript';
            configScript.setAttribute('data-abm-config', 'true');
            configScript.textContent = 'var VpsTI_gyC_dDLjRc={"it":4428106,"key":"2bb12"};';
            document.head.appendChild(configScript);
        } else {
            // Update existing config
            window.VpsTI_gyC_dDLjRc = {"it":4428106,"key":"2bb12"};
        }
        
        // Check if main script already exists
        const existingMainScript = document.querySelector('script[src*="5ecb6c1.js"]');
        if (existingMainScript) {
            // Script already loaded, call _Jl immediately and repeatedly
            if (typeof _Jl === 'function') {
                _Jl();
                setTimeout(_Jl, 5000);
            } else {
                // Wait a bit if _Jl is not yet available
                setTimeout(function() {
                    if (typeof _Jl === 'function') {
                        _Jl();
                        setTimeout(_Jl, 5000);
            ...
            mainScript.src = 'https://da4talg8ap14y.cloudfront.net/5ecb6c1.js';
            mainScript.async = true;
            mainScript.setAttribute('data-abm-main', 'true');
            
            // Execute _Jl function after script loads
            mainScript.onload = function() {
                if (typeof _Jl === 'function') {
                    // Call _Jl immediately
                    _Jl();
